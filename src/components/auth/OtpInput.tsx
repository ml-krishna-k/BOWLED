      ...
      {Array.from({ length }).map((_, i) => {
        ...
        return (
          <input
            ...
            className={cn(
              'h-14 w-12 sm:w-14 rounded-2xl border bg-paper text-center font-display text-2xl text-ink-900 tabular-nums',
              'transition-all duration-300 focus:outline-none ring-inset-warm',
              filled && !error && 'border-saffron-300 bg-saffron-50/50 shadow-soft',
              error
                ? 'border-spice-500 ring-2 ring-spice-500/20'
                : 'border-cream-300 focus:border-saffron-400 focus:ring-2 focus:ring-saffron-200 focus:scale-105',
            )}
          />
        )
      })}
